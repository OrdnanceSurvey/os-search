...
d.options=d.options||{},d.options.providers=d.options.providers||[],d.allProviders=d.options.providers.reduce(function(a,b){return b.hasOwnProperty("providers")?a.concat(b.providers):(a.push(b),a)},[]),d.options.placeholder=d.options.hasOwnProperty("placeholder")?d.options.placeholder:"Start typing to search",d.searchResults={},d.options.searchInputNumber=d.options.hasOwnProperty("searchInputNumber")?d.options.searchInputNumber:2,
...
// ---------- variables setup end -----------
var h=function(a,b){return c.Observable.create(function(c){try{var d=a.call(this,b);c.onNext(d),c.onCompleted()}catch(e){c.onError(e)}})},i=function(a,d){var e=a.url;angular.isFunction(a.url)&&(e=a.url(d));var f={params:angular.copy(a.params),data:angular.copy(a.data),dataType:a.dataType,url:e,method:a.method};
// inject search term into params and data
for(var g in f.params)f.params[g]=f.params[g].replace("%s",d);for(g in f.data)f.data[g]=f.data[g].replace("%s",d);return c.Observable.fromPromise(b(f))},j=function(a,b){
...
angular.isFunction(d.then)?d.then(function(a){c.onNext(a),c.onCompleted()})["catch"](function(a){c.onError(a)}):(c.onNext(d),c.onCompleted())}catch(e){c.onError(e)}})},k=function(a,b){
// check if provider is 'function' type
// check if provider is 'function' type
// else assume provider is 'AJAX' type
return a.hasOwnProperty("fn")?j(a.fn,b):a.hasOwnProperty("preTransformResult")?h(a.preTransformResult,b).flatMap(function(b){return i(a,b)}):i(a,b)},l=a(d,"searchInput").debounce(d.options.buffer||g).map(function(a){return a.newValue}).distinctUntilChanged();// ignore duplicate searches if value didn't change since last search
...
d.searchResults[a.id].results=[]}),a&&a.length&&a.length>d.options.searchInputNumber}).subscribe(function(a){var b=d.allProviders.map(function(b){d.$apply(function(){d.searchResults[b.id].inProgress=!0,d.searchResults[b.id].results=[],d.searchResults[b.id].error=void 0,d.searchResults[b.id].received=1/0});var c=k(b,a);return c.providerId=b.id,c.term=a,c.sent=new Date,c.config=b,c});b.forEach(function(a){a.subscribe(function(b){
...
d.selectResult=function(a,b){b&&b.call(null,a),d.searchInput=a.text},d.checkToShowResults=function(a){return a>d.options.searchInputNumber},d.aggregateLength=function(a){return a.providers.reduce(function(a,b){return b&&d.searchResults[b.id]?a+(d.searchResults[b.id].results||[]).length:a},0)}}}};angular.module("osel-search").directive("oselSearch",a.concat([b]))}();